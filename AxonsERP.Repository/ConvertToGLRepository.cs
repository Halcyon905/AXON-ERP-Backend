...
namespace AxonsERP.Repository
{
    internal class ConvertToGLRepository : RepositoryBase, IConvertToGLRepository
    {
        ...
        {
            var ConvertToGLList = Connection.Query<ConvertToGL>(@"SELECT
        ...
        public ConvertToGL GetSingleConvertToGL(ConvertToGLForGetSingle convertToGLForGetSingle)
        {
            string query = @"SELECT
                            ...
                            WHERE
                                ...
                                GL.OPERATION_CODE = :operationCode AND
                                GL.SUB_OPERATION = :subOperation AND
                                GL.BUSINESS_TYPE = :businessType AND
                                GL.DOC_TYPE = :docType AND
                                GL.TRN_CODE = :trnCode AND
                                GL.GROUP_ACCOUNT = :groupAccount AND
                                GL.EFFECTIVE_DATE = :effectiveDate
                            ORDER BY
                                GL.COMPANY,
                                GL.OPERATION_CODE,
                                GL.SUB_OPERATION,
